...
+
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        const organisedEvents = await fetch(`http://localhost:5000/organisers/getCreatedEvents`,{
          method: "GET",
          headers: {
            "Authorization": `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json"
        ...
        const organisedData = await organisedEvents.json();
        console.log("Organised Events:", organisedData);

        const filteredOrganisedData = organisedData.filter(item => item.event !== null);
        console.log(organisedData);  
        setAllEvents(filteredOrganisedData);        
        

        const registeredEvents = await fetch(`http://localhost:5000/organisers/getOrganiserEvent`,{
          method: "GET",
          headers: {
            "Authorization": `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json"
        ...
        const registeredData = await registeredEvents.json();
        console.log("Registered Events:", registeredData);
        const filteredRegisteredData = registeredData.filter(item => item.event !== null);
        setEvents(filteredRegisteredData);
  ...
  return (
    <div className="p-6 min-h-screen bg-[#121212] text-white">
      ...
      {activeTab == "organised" && allEvents.length > 0 ? (
